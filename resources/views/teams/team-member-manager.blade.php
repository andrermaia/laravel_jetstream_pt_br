<div>
    @if (Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-form-section submit="addTeamMember">
                <x-slot name="title">
                    {{ __('team-page.add_member') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('team-page.add_member_text_01') }}
                </x-slot>

                <x-slot name="form">
                    <div class="col-span-6">
                        <div class="max-w-xl text-sm text-gray-600">
                            {{ __('team-page.add_member_text_02') }}
                        </div>
                    </div>

                    <!-- Member Email -->
                    <div class="col-span-6 sm:col-span-4">
                        <x-label for="email" value="{{ __('team-page.email') }}" />
                    ...
                    @if (count($this->roles) > 0)
                        <div class="col-span-6 lg:col-span-4">
                            <x-label for="role" value="{{ __('team-page.role') }}" />
    ...
    @if ($team->teamInvitations->isNotEmpty() && Gate::check('addTeamMember', $team))
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('team-page.pending_invite') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('team-page.pending_invite_text_01') }}
                </x-slot>
    ...
    @if ($team->users->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('team-page.team_members') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('team-page.team_members_text') }}
                </x-slot>
    ...
    <x-confirmation-modal wire:model.live="confirmingLeavingTeam">
        <x-slot name="title">
            {{ __('team-page.leave_team') }}
        </x-slot>

        <x-slot name="content">
            {{ __('team-page.leave_team_text_01') }}
        </x-slot>
    ...
    <x-confirmation-modal wire:model.live="confirmingTeamMemberRemoval">
        <x-slot name="title">
            {{ __('team-page.remove_team') }}
        </x-slot>

        <x-slot name="content">
            {{ __('team-page.leave_team_text_02') }}
        </x-slot>
