<div>
    ...
    <x-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('api-page.create_api') }}
        </x-slot>

        <x-slot name="description">
            {{ __('api-page.allow_api') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-label for="name" value="{{ __('api-page.token_name') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-label for="permissions" value="{{ __('api-page.permissions') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-action-section>
                <x-slot name="title">
                    {{ __('api-page.manage_api') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('api-page.delete_text') }}
                </x-slot>
    ...
    <x-dialog-modal wire:model.live="displayingToken">
        <x-slot name="title">
            {{ __('api-page.api_token') }}
        </x-slot>

        <x-slot name="content">
            <div>
                {{ __('api-page.copy_api') }}
            </div>
    ...
    <x-confirmation-modal wire:model.live="confirmingApiTokenDeletion">
        <x-slot name="title">
            {{ __('api-page.delete_api') }}
        </x-slot>

        <x-slot name="content">
            {{ __('api-page.delete_api_text') }}
        </x-slot>
